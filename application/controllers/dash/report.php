...
class report extends CI_Controller {
	...
	public function report_wamas_release()
	{
		$data = array();
		$data['menu'] = $this->load->view('nav/nav_menu', '', true);
		$this->load->view('dash/dash_report' ,$data);

	}



	public function report_wamas_release_data(){
		$dash_log_report_release_wamas['data'] = $this->Model_Dash->dash_log_report_release_wamas();
		...
		for ($i = 0; $i < sizeof($dash_log_report_release_wamas['data']) ; $i++) {

			array_push($labels, $dash_log_report_release_wamas['data'][$i]['release_wamas_log_datetime']);
			array_push($dash_data, $dash_log_report_release_wamas['data'][$i]['release_wamas_log_count_record']);

			$sub_array = array();
			$sub_array[] = $i+1;
			$sub_array[] = $dash_log_report_release_wamas['data'][$i]['release_wamas_log_count_record'];
			$sub_array[] = $dash_log_report_release_wamas['data'][$i]['release_wamas_log_file_url'];
			$sub_array[] = $dash_log_report_release_wamas['data'][$i]['release_wamas_log_datetime'];
			$sub_array[] = '<button type="button" class="btn btn-outline-primary" onclick="load(\''.$dash_log_report_release_wamas['data'][$i]['release_wamas_log_file_url'].'\')">Download</button>';
			$data_table[] = $sub_array;

	   }
	   $dash_log_report_release_wamas['labels'] = $labels;
	   $dash_log_report_release_wamas['dash_data'] = $dash_data;

	//    echo "<pre>".print_r($data_table,true)."</pre>";
	   $output = array(
			"data" => $data_table,
			"dash" => $dash_log_report_release_wamas
	   );
	...
	public function report_wamas(){
		...
		$data = array();
		$data['menu'] = $this->load->view('nav/nav_menu', '', true);
		// $this->load->view('dash/dash_wamas' ,$data);
		$this->load->view('dash/dash_wamas_og002' ,$data);
	}

	public function report_wamas_data($customer_type){
		
		$dash_wamas_og002 = $this->Model_Dash->dash_wamas_og002($customer_type);
	...
	public function dash_wamas_og002_monthly_data($month,$customer_type){
		// $month = '2023-10-01';
		$dash_wamas_og002_daily = $this->Model_Dash->dash_wamas_og002_daily($month,$customer_type);
	...
	public function dash_wamas_og002_table($month,$customer_type){
		...
		$data['customer_type'] = $customer_type;
 		$this->load->view('dash/dash_wamas_og002_tabel',$data);
	}
